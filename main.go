...
func main() {
	...
	flag.StringVar(&tpl, "tpl", "", "Output template ('json' = default, 'pretty')")
	flag.StringVar(&sorter, "sort", "", "Sort ('' = default, 'star', 'fork')")
	flag.IntVar(&page, "page", 0, "Pages to crawl (0 == all)")
	flag.StringVar(&after, "after", "", "Hash of offset to be set in `dependents_after` query param")
	flag.Parse()
	identity := flag.Arg(0)
	c := &ghdeps.Crawler{
